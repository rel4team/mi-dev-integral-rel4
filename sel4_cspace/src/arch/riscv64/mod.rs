use sel4_common::{
    arch::maskVMRights, cap_rights::seL4_CapRights_t,
    structures::exception_t, structures_gen::{cap, cap_asid_control_cap, cap_asid_pool_cap, cap_cnode_cap, cap_domain_cap, cap_endpoint_cap, cap_frame_cap, cap_irq_control_cap, cap_irq_handler_cap, cap_notification_cap, cap_null_cap, cap_page_table_cap, cap_reply_cap, cap_tag, cap_thread_cap, cap_untyped_cap, cap_zombie_cap}, utils::{pageBitsForSize, ptr_to_mut},
    vm_rights::vm_rights_from_word, MASK,
};

use crate::{
	capability::{cap_arch_func, zombie::zombie_func},
	cte::deriveCap_ret, interface::cte_t};
...
pub fn arch_same_object_as(cap1: &cap, cap2: &cap) -> bool {
        ...
        && cap2.get_tag() == cap_tag::cap_frame_cap
    {
        return cap::to_cap_frame_cap(cap1).get_capFBasePtr() == cap::to_cap_frame_cap(cap2).get_capFBasePtr()
            ...
            && (cap::to_cap_frame_cap(cap1).get_capFIsDevice() == 0) == (cap::to_cap_frame_cap(cap2).get_capFIsDevice() == 0);
    }
