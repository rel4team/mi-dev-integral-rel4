use crate::PTE;
use sel4_common::{
    fault::lookup_fault_t, sel4_config::{asidHighBits, asidLowBits, IT_ASID}, structures::exception_t, structures_gen::{asid_map, asid_map_Splayed, asid_map_asid_map_none, asid_map_asid_map_vspace, asid_map_tag}, utils::{convert_to_mut_type_ref, convert_to_option_mut_type_ref}, BIT, MASK
};
...
pub fn delete_asid(asid: usize, vspace: *mut PTE, cap: &cap_t) -> Result<(), lookup_fault_t> {
    ...
    if let Some(pool) = ptr {
		...
		match asidmap.splay(){
			asid_map_Splayed::asid_map_vspace(data)=>{
				if data.get_vspace_root() as usize == vspace as usize{
