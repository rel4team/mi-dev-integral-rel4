...
use sel4_common::{arch::ArchReg, structures_gen::{cap_frame_cap, cap_page_table_cap}};
#[cfg(target_arch = "aarch64")]
use sel4_common::BIT;
...
pub fn invoke_page_table_unmap(capability: &mut cap_page_table_cap) -> exception_t {
    if capability.get_capPTIsMapped() != 0 {
        ...
        unmap_page_table(capability.get_capPTMappedASID() as usize, capability.get_capPTMappedAddress() as usize, pt);
...
pub fn invoke_page_unmap(frame_slot: &mut cte_t) -> exception_t {
    if unsafe { core::mem::transmute::<cap, cap_frame_cap>(frame_slot.capability) }.get_capFMappedASID() as usize != asidInvalid {
        match unmapPage(
            unsafe { core::mem::transmute::<cap, cap_frame_cap>(frame_slot.capability) }.get_capFSize() as usize,
            unsafe { core::mem::transmute::<cap, cap_frame_cap>(frame_slot.capability) }.get_capFMappedASID() as usize,
            // FIXME: here should be frame_mapped_address.
            unsafe { core::mem::transmute::<cap, cap_frame_cap>(frame_slot.capability) }.get_capFMappedAddress() as usize,
            unsafe { core::mem::transmute::<cap, cap_frame_cap>(frame_slot.capability) }.get_capFBasePtr() as usize,
        ) {
    ...
    unsafe { core::mem::transmute::<cap, cap_frame_cap>(frame_slot.capability) }.set_capFMappedAddress(0);
    unsafe { core::mem::transmute::<cap, cap_frame_cap>(frame_slot.capability) }.set_capFMappedASID(asidInvalid as u64);
    exception_t::EXCEPTION_NONE
}
...
#[cfg(target_arch = "aarch64")]
pub fn invoke_page_map(asid: usize, capability: cap, pte: PTE, pt_slot: &mut PTE) -> exception_t {
    ...
    if unlikely(tlbflush_required) {
        assert!(asid < BIT!(16));
        invalidate_tlb_by_asid_va(asid, unsafe { core::mem::transmute::<cap, cap_frame_cap>(capability) }.get_capFMappedAddress() as usize);
    }
    exception_t::EXCEPTION_NONE
}
// #[cfg(target_arch = "aarch64")]
