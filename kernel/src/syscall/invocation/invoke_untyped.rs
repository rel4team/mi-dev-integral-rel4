...
use sel4_common::structures_gen::{cap, cap_cnode_cap, cap_endpoint_cap, cap_notification_cap, cap_thread_cap, cap_untyped_cap};
...
fn create_new_objects(
    ...
    for i in 0..dest_length {
        ...
        insert_new_cap(parent, dest_cnode.get_offset_slot(dest_offset + i), &capability);
    }
}
...
fn create_object(
    ...
    match obj_type {
        ...
        ObjectType::CapTableObject => cap_cnode_cap::new(user_size as u64, 0, 0, region_base as u64).unsplay(),
        ObjectType::NotificationObject => cap_notification_cap::new(0, 1, 1, region_base as u64).unsplay(),
        ObjectType::EndpointObject => cap_endpoint_cap::new(0, 1, 1, 1, 1, region_base as u64).unsplay(),
        ObjectType::UnytpedObject => cap_untyped_cap::new(0, device_mem as u64, user_size as u64, region_base as u64).unsplay(),
        _ => arch_create_object(obj_type, region_base, user_size, device_mem),
    }
}

pub fn reset_untyped_cap(srcSlot: &mut cte_t) -> exception_t {
    let prev_cap = &mut unsafe { core::mem::transmute::<cap, cap_untyped_cap>((*srcSlot).capability) };
    ...
    let device_mem = prev_cap.get_capIsDevice() as usize;
    if offset == 0 {
...
pub fn invoke_untyped_retype(
    ...
    let region_base = unsafe { core::mem::transmute::<cap, cap_untyped_cap>(src_slot.capability) }.get_capPtr();
    if reset {
    ...
    unsafe { core::mem::transmute::<cap, cap_untyped_cap>(src_slot.capability) }
        .set_capFreeIndex(GET_FREE_INDEX(region_base as usize, free_ref) as u64);
    create_new_objects(
        new_type,
        src_slot,
        dest_cnode,
