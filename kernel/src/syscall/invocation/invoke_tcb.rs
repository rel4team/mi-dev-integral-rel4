...
pub fn invoke_tcb_set_space(
    ...
    if same_object_as(&croot_new_cap, &croot_src_slot.capability) && same_object_as(&target_cap, &slot.capability)
    {
    ...
    if same_object_as(&vroot_new_cap, &vroot_src_slot.capability) && same_object_as(&target_cap, &slot.capability)
    {
