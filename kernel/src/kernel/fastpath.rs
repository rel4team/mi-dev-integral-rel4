...
use sel4_common::structures_gen::{
    cap, cap_cnode_cap, cap_endpoint_cap, cap_null_cap, cap_page_table_cap, cap_reply_cap, cap_tag,
};
use sel4_common::{
    fault::*,
    message_info::*,
...
pub fn lookup_fp(_cap: &cap, cptr: usize) -> cap {
    ...
    loop {
        let cap_data = unsafe { core::mem::transmute::<cap, cap_cnode_cap>(capability) };
        guardBits = cap_data.get_capCNodeGuardSize() as usize;
        radixBits = cap_data.get_capCNodeRadix() as usize;
        cptr2 = cptr << bits;
        capGuard = cap_data.get_capCNodeGuard() as usize;
        if likely(guardBits != 0) && unlikely(cptr2 >> (wordBits - guardBits) != capGuard) {
            return cap_null_cap::new().unsplay();
        }

        radix = cptr2 << guardBits >> (wordBits - radixBits);
        slot = unsafe { (cap_data.get_capCNodePtr() as *mut cte_t).add(radix) };
...
#[inline]
#[no_mangle]
pub fn isValidVTableRoot_fp(capability: &cap_page_table_cap) -> bool {
    // cap_capType_equals(cap, cap_page_table_cap) && cap.get_pt_is_mapped() != 0
    capability.unsplay().get_tag() == cap_tag::cap_page_table_cap
        && capability.get_capPTIsMapped() != 0
}
...
pub fn fastpath_call(cptr: usize, msgInfo: usize) {
    ...
    let ep_cap = unsafe { core::mem::transmute::<cap, cap_endpoint_cap>(ep_cap) };
    if unlikely(
    ...
    let new_vtable = unsafe {
        core::mem::transmute::<cap, cap_page_table_cap>(dest.get_cspace(tcbVTable).capability)
    };
    ...
    let badge = ep_cap.get_capEPBadge();
    fastpath_restore(badge as usize, msgInfo1, get_currenct_thread());
}
...
pub fn fastpath_reply_recv(cptr: usize, msgInfo: usize) {
    ...
    if unlikely(caller_cap.get_tag() != cap_tag::cap_reply_cap) {
        slowpath(SysReplyRecv as usize);
    }

    let caller = convert_to_mut_type_ref::<tcb_t>(
        unsafe { core::mem::transmute::<cap, cap_reply_cap>(*caller_cap) }.get_capTCBPtr() as usize,
    );
    ...
    let new_vtable = &unsafe {
        core::mem::transmute::<cap, cap_page_table_cap>(caller.get_cspace(tcbVTable).capability)
    };
